    ...
    <Transition name="dialog-backdrop" appear>
      ...
      >
        ...
        <Transition name="dialog-content" appear>
          <div
            v-if="visible"
            class="fnx-dialog-container"
            :class="[`fnx-dialog--${variant}`, { 'fnx-dialog--closing': isClosing }]"
            :style="dialogStyles"
            @click.stop
            ref="dialogRef"
          >
            <!-- Dialog Header -->
            <div class="fnx-dialog-header">
              <!-- Icon -->
              <div v-if="icon || hasDefaultIcon" class="fnx-dialog-icon" :class="`fnx-dialog-icon--${variant}`">
                <i :class="displayIcon"></i>
              </div>
              
              <!-- Title -->
              <h3 v-if="title" :id="titleId" class="fnx-dialog-title">
                {{ title }}
              </h3>
            ...
            <!-- Dialog Body -->
            <div class="fnx-dialog-body">
              <p :id="messageId" class="fnx-dialog-message">
                {{ message }}
              </p>
            </div>

            <!-- Dialog Footer -->
            <div class="fnx-dialog-footer">
              <button
                class="fnx-button fnx-button--secondary"
                @click="handleCancel"
                type="button"
                ref="cancelButtonRef"
              >
...
const variantConfigs: DialogVariantConfigs = {
  warning: {
    iconColor: '#f59e0b',
    confirmButtonClass: 'fnx-button--warning',
    borderColor: '#fbbf24',
  ...
  white: {
    iconColor: '#6b7280',
    confirmButtonClass: 'fnx-button--primary',
    borderColor: '#e5e7eb',
  ...
  danger: {
    iconColor: '#ef4444',
    confirmButtonClass: 'fnx-button--danger',
    borderColor: '#f87171',
...
.fnx-dialog-icon {
  ...
  &.fnx-dialog-icon--warning {
    background: #fef3c7;
    color: #f59e0b;
  }
  
  &.fnx-dialog-icon--white {
    background: #f3f4f6;
    color: #6b7280;
  }
  
  &.fnx-dialog-icon--danger {
    background: #fee2e2;
    color: #ef4444;
  }
}
