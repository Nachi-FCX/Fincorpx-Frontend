<template>
  <div class="fnx-field" :class="fieldClass">
    ...
    <!-- MultiSelect wrapper -->
    <div class="fnx-multiselect-wrapper">
      <!-- MultiSelect element -->
      <MultiSelect
        :id="multiselectId"
        ref="multiselectRef"
      ...
      <!-- Loading spinner -->
      <div v-if="loading" class="fnx-multiselect-loading">
...
const multiselectId = computed(() => `fnx-multiselect-${props.name}`)
...
const multiselectClasses = computed(() => [
  'fnx-multiselect',
  `fnx-multiselect--${props.size}`,
  {
    'fnx-multiselect--error': !!errorMessage.value,
    'fnx-multiselect--success': meta.valid && meta.touched && !errorMessage.value,
    'fnx-multiselect--loading': props.loading,
    'fnx-multiselect--chip-display': props.display === 'chip'
  }
])
...
<style scoped>
.fnx-multiselect-wrapper {
  position: relative;
}

.fnx-multiselect-loading {
  position: absolute;
...
/* State variants */
.fnx-multiselect--error {
  border-color: var(--fnx-error-500);
}

.fnx-multiselect--success {
...
.fnx-multiselect--chip-display :deep(.p-multiselect-token) {
  ...
  margin: var(--fnx-spacing-xs);
}

.fnx-multiselect--chip-display :deep(.p-multiselect-token-icon) {
