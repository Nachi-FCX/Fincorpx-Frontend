...
// Props with defaults
const props = withDefaults(defineProps<FnxDialogProps>(), {
  visible: false,
  modal: true,
  position: 'center',
  draggable: false,
...
// Emits
const emit = defineEmits<FnxDialogEmits>()
...
const computedDialogClass = computed(() => [
  'fnx-dialog',
  `fnx-dialog--${props.size}`,
  `fnx-dialog--${props.theme}`,
  {
    'fnx-dialog--rounded': props.rounded,
    'fnx-dialog--shadow': props.shadow,
    'fnx-dialog--maximized': isMaximized.value
  },
  props.class
])
