<template>
  <div class="fnx-datatable" :class="tableWrapperClasses">
    <!-- Enhanced Table Header -->
    <div class="fnx-datatable__header" v-if="showHeader">
      <div class="fnx-datatable__header-content">
        <!-- Left side actions -->
        <div class="fnx-datatable__header-left">
          <FnxButton 
            v-if="clearShow" 
            label="Clear Filters" 
            ...
            size="small"
            @click="clearFilter"
            class="fnx-datatable__clear-btn"
          />
          ...
          <!-- <template v-for="button in leftActionButtons" :key="button.action">
            <FnxButton 
              ...
              class="fnx-datatable__action-btn"
            />
          </template> -->
        ...
        <!-- Right side actions -->
        <div class="fnx-datatable__header-right">
          <!-- Add Button -->
          <FnxButton 
            v-if="showAddButton" 
          ...
          <FnxButton 
            ...
            class="fnx-datatable__refresh-btn"
          />
          ...
          <FnxButton 
            ...
            class="fnx-datatable__import-btn"
          />
          ...
          <div class="fnx-datatable__search-wrapper" v-if="showGlobalFilter">
            <FnxInputtext
              name="globalSearch"
              ...
              class="fnx-datatable__search-input"
            />
          </div>
          ...
          <div class="fnx-datatable__export-wrapper" v-if="exportStatus==0 && showExport">
            <FnxButton 
              icon="pi pi-download" 
              severity="secondary" 
              size="small"
              @click="toggle"
              class="fnx-datatable__export-btn"
              :class="{ 'fnx-datatable__export-btn--active': showExportMenu }"
            />
            
            <!-- Enhanced Export Menu -->
            <Transition name="export-menu">
              <div v-if="showExportMenu" class="fnx-datatable__export-overlay" @click="hideExportMenu">
                <div class="fnx-datatable__export-menu" @click.stop>
                  <div class="fnx-datatable__export-menu-header">
                    <span>Export Options</span>
                  </div>
                  <div class="fnx-datatable__export-menu-item" @click="exportToPDF">
                    <i class="pi pi-file-pdf"></i>
                    <div class="fnx-datatable__export-menu-item-content">
                      <span class="fnx-datatable__export-menu-item-title">Export as PDF</span>
                      <span class="fnx-datatable__export-menu-item-desc">Download table data as PDF</span>
                    </div>
                  </div>
                  <div class="fnx-datatable__export-menu-item" @click="exportToExcel">
                    <i class="pi pi-file-excel"></i>
                    <div class="fnx-datatable__export-menu-item-content">
                      <span class="fnx-datatable__export-menu-item-title">Export as Excel</span>
                      <span class="fnx-datatable__export-menu-item-desc">Download table data as XLSX</span>
                    </div>
                  </div>
                </div>
              </div>
    ...
    <div v-if="loading && showLoadingOverlay" class="fnx-datatable-loading-overlay" :style="{ opacity: loadingOverlayOpacity }">
      <div class="fnx-datatable-loading-content">
        <div class="fnx-datatable-loading-spinner"></div>
        <div class="fnx-datatable-loading-message">{{ loadingMessage }}</div>
      </div>
    </div>
    ...
    >
      ...
      <Column v-for="col in props.columns"
        ...
        <template #body="slotProps">
          <div v-if="props.loading && props.showSkeleton" 
               :class="['fnx-datatable__skeleton-item', `fnx-datatable__skeleton-item--${props.skeletonAnimation}`]">
            <div class="fnx-datatable__skeleton-bar"></div>
          </div>
      ...
      <Column v-if="props.showActionsColumn"
        ...
        <template #body="slotProps">
          <slot name="body-actions" v-bind="slotProps">
            <div class="fnx-datatable__actions-placeholder">
      ...
      <template #empty>
        <div class="fnx-datatable-empty-state">
          <div class="fnx-datatable-empty-icon">
            <i :class="emptyIcon || 'pi pi-inbox'"></i>
          </div>
          <div class="fnx-datatable-empty-message">
            {{ emptyMessage || 'No records found' }}
          </div>
          <div class="fnx-datatable-empty-description" v-if="emptyDescription">
            {{ emptyDescription }}
          </div>
        </div>
      </template>
      ...
      <template #loading v-if="!showLoadingOverlay">
        <div class="fnx-datatable-loading-inline">
          <div class="fnx-datatable-loading-spinner"></div>
    ...
    <!-- Error State -->
    <div v-if="errorState?.show" class="fnx-datatable__error-state">
      <div class="fnx-datatable__error-content">
        <i :class="errorState.icon" class="fnx-datatable__error-icon"></i>
        <h3 class="fnx-datatable__error-title">{{ errorState.title }}</h3>
        <p class="fnx-datatable__error-message">{{ errorState.message }}</p>
        <div class="fnx-datatable__error-actions">
          <FnxButton
            ...
            class="fnx-datatable__error-retry"
          />
          <FnxButton
            ...
            class="fnx-datatable__error-support"
          />
        </div>
      </div>
    </div>
...
const props = withDefaults(defineProps<FnxDataTableProps>(), {
  rows: 10,
  first: 0,
  totalRecords: 0,
  transactionStatus: 0,
...
const onFilter = (event: FnxDataTableFilterEvent) => {
...
const tableWrapperClasses = computed(() => [
  ...
  {
    // Size variants
    'fnx-datatable--small': props.size === 'small',
    'fnx-datatable--medium': props.size === 'medium',
    'fnx-datatable--large': props.size === 'large',
    
    // Theme variants
    'fnx-datatable--default': props.theme === 'default',
    'fnx-datatable--minimal': props.theme === 'minimal',
    'fnx-datatable--bordered': props.theme === 'bordered',
    'fnx-datatable--compact': props.theme === 'compact',
    
    // Style options
    'fnx-datatable--rounded': props.rounded,
    'fnx-datatable--raised': props.raised,
    'fnx-datatable--striped': props.stripedRows,
    'fnx-datatable--gridlines': props.showGridlines,
    'fnx-datatable--hoverable': props.rowHover,
    
    // State classes
    'fnx-datatable--loading': props.loading,
    'fnx-datatable--responsive': props.responsive
  }
])
